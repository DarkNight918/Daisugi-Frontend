...
const DeFiTVL: FC = () => {
  const [feeData, setFeeData] = useState<responseType | undefined>();
  const [yieldData, setYieldData] = useState<responseType | undefined>();
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-10">
        ...
        <Link href="/defi/fee">
          <XBox isBackground={true} gap={true} header="Fees & Revenue" isLoading={isLoading}>
            {feeData &&
              feeData.map((item: any, index: any) => (
                <CoinListBox
                  key={index}
                  // imgURL={item.logo}
